...
mSleep				--延迟，参数int毫秒
touchDown			--按下，参数1 x，参数3 y，返回值 int:index
touchUp				--弹起，参数1 touchDown的返回值index，参数2 x，参数3 y
touchMove			--移动，参数1 touchDown的返回值index，参数2 x，参数3 y
...
ocrText				--ocr光学识别文本，参数1 table{x1,y1,x2,y2}，参数2 string 白名单，返回值string
]]--
...
function click(x,y)
	...
	local pointId = touchDown(ranx,rany);
	mSleep(s);
	touchUp(pointId,ranx,rany);
end

function move(orien,x,y,dis)--0向下拉，1向上拉，2向右拉，3向左拉
	local ranx = x + math.random(-10,10);
	local rany = y + math.random(-10,10);
	local s = math.random(300,400);
	local pointId = touchDown(ranx,rany);
	mSleep(s);
	if orien==0 then
	 	-- while rany<=y+dis do
	 	-- 	ranx = ranx + math.random(-5,5);
	 	-- 	rany = rany + math.random(10,20);
	 	-- 	s = math.random(20,50);
	 	-- 	touchMove(pointId,ranx,rany);
	 	-- 	mSleep(s);
	 	-- end
 		ranx = ranx + math.random(-5,5);
 		rany = rany + math.random(dis-10,dis+10);
	 	touchMove(pointId,ranx,rany);
	elseif orien==1 then
	 	-- while rany>=y-dis do
	 	-- 	ranx = ranx + math.random(-5,5);
	...
	elseif orien==2 then
	 	-- while ranx<=x+dis do
	 	-- 	ranx = ranx + math.random(10,20);
	 	-- 	rany = rany + math.random(-5,5);
	 	-- 	s = math.random(20,50);
	 	-- 	touchMove(pointId,ranx,rany);
	 	-- 	mSleep(s);
	 	-- end
	 	ranx = ranx + math.random(dis-10,dis+10);
 		rany = rany + math.random(-5,5);
	 	touchMove(pointId,ranx,rany);
	elseif orien==3 then
	 	-- while ranx>=x-dis do
