...
-(NSString *)ocrText:(int)x1 y1:(int)y1 x2:(int)x2 y2:(int)y2 whitelist:(NSString *)whitelist{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
//    UIImage *newImage = [UIImage imageWithCGImage:newImageRef];
    UIImage *newImage =[[NSOpen alloc] opencvDisposeImage:[UIImage imageWithCGImage:newImageRef]];
    //写入相册
...
-(NSInteger)touchDown:(int)x y:(int)y{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    NSInteger pointId = [PTFakeMetaTouch fakeTouchId:[PTFakeMetaTouch getAvailablePointId] AtPoint:CGPointMake(ceil(x*0.5),ceil(y*0.5)) withTouchPhase:UITouchPhaseBegan];
    [pool drain];
    return pointId;
}
...
-(void)touchUp:(int)index x:(int)x y:(int)y{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    if(index != 0)
        [PTFakeMetaTouch fakeTouchId:index AtPoint:CGPointMake(ceil(x*0.5),ceil(y*0.5)) withTouchPhase:UITouchPhaseEnded];
    [pool drain];
}
...
-(void)touchMove:(int)index x:(int)x y:(int)y{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    if(index != 0)
        [PTFakeMetaTouch fakeTouchId:index AtPoint:CGPointMake(ceil(x*0.5),ceil(y*0.5)) withTouchPhase:UITouchPhaseMoved];
    [pool drain];
}
    ...
    for (int m=y1; m<y2; m++) {
        for (int n=x1; n<x2; n++) {
            ...
            if(fabsf(red-rMain)<=(rMain1+simi) && fabsf(green-gMain)<=(gMain1+simi) && fabsf(blue-bMain)<=(bMain1+simi) ){
                //NSLog(@"x=%d,y=%d",n,m);
                if(mutableArray.count == 1){
                    p = [NSString stringWithFormat:@"%d,%d",n,m];
                    return p;
                }else{
                    int max=0;
                    float rValue=0,gValue=0,bValue=0;
...
//                        NSLog(@"找到了完全%@",p);
                        if([p isEqual:@""]){
                            ...
                            p = [NSString stringWithFormat:@"%@|%d,%d",p,n,m];
                        }
