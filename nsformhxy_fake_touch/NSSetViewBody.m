...
-(void)craeteView0{
    ...
    UISlider * smSlider3 = [self createSlider:view point:CGPointMake(addView[view].center.x*0.7, addView[view].center.y*0.58) min:0 max:30 width:addView[view].frame.size.width*0.5 tag:915];
    ...
    if(smSlider3.value>0){
        str = [NSString stringWithFormat:@"秘境几关结束 :%.0f", smSlider3.value];
    }else{
        str = @"秘境几关结束 :死亡结束";
    }
    ...
    [self createSeg:view point:CGPointMake(addView[view].center.x*0.79, addView[view].center.y*0.78) width:addView[view].frame.size.width*0.4 array:@[@"一次",@"二次",@"三次"] str:@"主线红尘死亡结束" tag:916];
...
-(void)craeteView2{
    ...
    UISlider * smSlider2 = [self createSlider:view point:CGPointMake(addView[view].center.x*0.07, addView[view].center.y*0.75) min:0 max:1000 width:addView[view].frame.size.width*0.5 tag:603];
    ...
    if(smSlider2.value>0){
        str = [NSString stringWithFormat:@"超过 :%.0f0 银币去传说", smSlider2.value];
    }else{
        str = @"超过 :0 银币去传说";
    }
    ...
    UISlider * smSlider4 = [self createSlider:view point:CGPointMake(addView[view].center.x*0.7, addView[view].center.y*0.75) min:0 max:1000 width:addView[view].frame.size.width*0.5 tag:605];
    ...
    if(smSlider4.value>0){
        str = [NSString stringWithFormat:@"超过 :%.0f0 银币结束任务", smSlider4.value];
    }else{
        str = @"超过 :0 银币结束任务";
    }
...
-(void)sliderValueChanged:(UISlider *)sl{
    ...
    switch (sl.tag) {
        ...
        case 603:
            if(sl.value>1){
                str = [NSString stringWithFormat:@"超过 :%.0f0 银币去传说", sl.value];
            }else{
                str = @"超过 :0 银币去传说";
            }
            sliderLabel[6].text = str;
        ...
        case 605:
            if(sl.value>1){
                str = [NSString stringWithFormat:@"超过 :%.0f0 银币结束任务", sl.value];
            }else{
                str = @"超过 :0 银币去传说";
            }
            sliderLabel[8].text = str;
...
-(void)buttonClick:(UIButton *)button{
    if(button.tag == 100){
        NSDictionary *reply;
        reply = [nsServer sendMessageAndReceiveReplyName:@"NSTEST" userInfo:nil];
        
        NSString *object = [reply objectForKey:@"MyTest"];
//        [self setController:@"设置中ing..."];
        [self setController:object];
        if(nsthread_create){
            nsthread_pause = true;
